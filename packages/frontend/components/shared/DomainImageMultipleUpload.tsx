...
export interface DomainImageUploadProps extends DropBoxStyleProps {
  count: number;
  onImageFinal: (imgSrcs : string[], imgBlob : Blob) => void;
  justifyContent? : string;
}

export function DomainImageMultipleUpload (props : DomainImageUploadProps) {
  
  const theme = useTheme();

  const [imgBlob, setImgBlob] = React.useState<Blob | null>(null);
  const [croppedImgUrls, setCroppedImgUrls] = React.useState<Array<{id : number, url : string}>>([]);
  ...
  const handleOnImgSelect = React.useCallback((filelist : FileList | null, removalIndex: number) => {
    console.log("here");
    if (filelist) {
      console.log(croppedImgUrls);
      setCroppedImgUrls(prev => [...prev, {id : prev.length, url : URL.createObjectURL(filelist[0])}]);
      setImgBlob(filelist[0]);
      setCropping(true);
    } else {
      setCroppedImgUrls(prev => {
        return Object.entries(prev).filter(([key, value]) => value.id !== removalIndex).map(([key, value]) => value);
      });
    }
  }, [croppedImgUrls])
  ...
  const handleCropSelection = async () => {
    ...
    setCroppedImgUrls(prev => {
      ...
      const filtered = Object.entries(prev).filter((_, idx, arr) => idx !== arr.length-1).map(([key, value]) => value);
      return [...filtered, {id : length, url : newImgSrc}]
    });
    setCropping(false);
  }

  const handleCropCancel = (removalIndex : number) => {
    setImgBlob(null);
    // setCroppedImgUrls(prev => {
    //   return Object.entries(prev).filter((_, idx, arr) => idx !== arr.length-1).map(([key, value]) => value)
    // });
    setCroppedImgUrls(prev => {
      return Object.entries(prev).filter(([key, value]) => value.id !== removalIndex).map(([key, value]) => value);
    });
  ...
  React.useEffect(() => {
    if (imgBlob && croppedImgUrls && !cropping) {
      props.onImageFinal(croppedImgUrls.map(({url}) => url), imgBlob);      
    }
  }, [imgBlob, croppedImgUrls, cropping]);
    ...
    <>
      ...
      >
        <div style={{display : "flex", flexDirection : "column", alignItems :"center", gap : 10, backgroundColor: "#fff", width : "50vw"}}>
          <CropContainer>
            <Cropper 
              image={croppedImgUrls.at(-1)?.url as string}
              aspect={1 / 1}
              crop={crop}
              rotation={rotation}
              zoom={zoom}
          ...
          <Button 
            color="error" 
            onClick={() => handleCropCancel(croppedImgUrls.length - 1)}
            sx={{
              textTransform : "none"
            }}
          >
      ...
      <Grid container spacing={1} justifyContent={props.justifyContent || "left"}> 
        <Grid item xs={4}>
          <DropBox
            backgroundColor={theme.palette.primary.light}
